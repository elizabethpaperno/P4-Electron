...
def alcohol_yn():
    ...
    while (i < res.len()):
        if(res[i] in alc_dict['address'][i]):
            query_usersdb(f"""UPDATE customers SET alcohol = ? WHERE address = ?;""", True, res[i])
            query_usersdb(f"""UPDATE customers SET mode = ? WHERE address = ?;""", alc_dict['mode'][i], res[i])
        else:
            query_usersdb(f"""UPDATE customers SET alcohol = ? WHERE address = ?;""", False, res[i])